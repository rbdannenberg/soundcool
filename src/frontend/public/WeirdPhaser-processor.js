...
class WeirdPhaserProcessor extends AudioWorkletProcessor {
    ...
    static atob(sBase64, nBlocksSize) {
        if (typeof atob === "function") {
            ...
            for (
                ...
                nMod4 = nInIdx & 3;
                nUint24 |=
                    WeirdPhaserProcessor.b64ToUint6(
    ...
    static get parameterDescriptors() {
        ...
        var params = [];
        WeirdPhaserProcessor.parse_ui(
            JSON.parse(getJSONWeirdPhaser()).ui,
            params,
            WeirdPhaserProcessor.parse_item1
        );
        return params;
    }
    ...
    constructor(options) {
        ...
        this.output_handler = function(path, value) {
        ...
        this.WeirdPhaser_instance = new WebAssembly.Instance(
            WeirdPhaserProcessor.wasm_module,
        ...
        this.audio_heap_ptr_outputs =
            this.audio_heap_ptr_inputs + this.numIn * this.ptr_size;

        // Setup buffer offset
        this.audio_heap_inputs =
            this.audio_heap_ptr_outputs + this.numOut * this.ptr_size;
        this.audio_heap_outputs =
            this.audio_heap_inputs +
            this.numIn * WeirdPhaserProcessor.buffer_size * this.sample_size;
        ...
        this.update_outputs = function() {
            if (
                this.outputs_items.length > 0 &&
                this.output_handler &&
                this.outputs_timer-- === 0
            ) {
                this.outputs_timer = 5;
                for (var i = 0; i < this.outputs_items.length; i++) {
                    this.output_handler(
                        this.outputs_items[i],
                        this.HEAPF32[this.pathTable[this.outputs_items[i]] >> 2]
                    );
                }
            }
        };
        ...
        this.initAux = function() {
            ...
            if (this.numIn > 0) {
                ...
                for (i = 0; i < this.numIn; i++) {
                    this.HEAP32[(this.ins >> 2) + i] =
                        this.audio_heap_inputs +
                        WeirdPhaserProcessor.buffer_size * this.sample_size * i;
                }

                // Prepare Ins buffer tables
                var dspInChans = this.HEAP32.subarray(
                    this.ins >> 2,
                    (this.ins + this.numIn * this.ptr_size) >> 2
                );
                for (i = 0; i < this.numIn; i++) {
                    this.dspInChannnels[i] = this.HEAPF32.subarray(
                        dspInChans[i] >> 2,
                        (dspInChans[i] +
                            WeirdPhaserProcessor.buffer_size *
                                this.sample_size) >>
                            2
                    );
                }
            ...
            if (this.numOut > 0) {
                ...
                var dspOutChans = this.HEAP32.subarray(
                    this.outs >> 2,
                    (this.outs + this.numOut * this.ptr_size) >> 2
                );
                for (i = 0; i < this.numOut; i++) {
            ...
            WeirdPhaserProcessor.parse_ui(
                this.json_object.ui,
                this,
                WeirdPhaserProcessor.parse_item2
            );
    ...
    process(inputs, outputs, parameters) {
        ...
        if (input !== undefined) {
            for (
                var chan = 0;
                chan < Math.min(this.numIn, input.length);
                ++chan
            ) {
        ...
        if (output !== undefined) {
            ...
            for (
                var chan = 0;
                chan < Math.min(this.numOut, output.length);
                ++chan
            ) {
...
try {
    if (WeirdPhaserProcessor.wasm_module == undefined) {
        WeirdPhaserProcessor.wasm_module = new WebAssembly.Module(
            WeirdPhaserProcessor.atob(getBase64CodeWeirdPhaser())
        );
        registerProcessor("WeirdPhaser", WeirdPhaserProcessor);
    }
} catch (e) {
    console.log(e);
    console.log("Faust WeirdPhaser cannot be loaded or compiled");
}
