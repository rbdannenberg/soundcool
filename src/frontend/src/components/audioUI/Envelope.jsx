...
class Envelope extends React.Component {
  ...
  checkCanvasLocation(e) {
    ...
    this.props.changeBlock(this.props.blockInfo.id, "envelope", [x, y],{num: this.props.blockInfo.pointCount});
    // pointCount ++
    this.props.changeBlock(this.id, "pointCount", this.props.blockInfo.pointCount + 1);
    this.renderLines();
  }
  ...
  renderLines() {
    ...
    for (let i = 0; i < sortedPoints.length - 1; i++) {
      canvasCtx.beginPath();
      canvasCtx.fillRect(sortedPoints[i][0] + left, sortedPoints[i][1], 3, 3);
      canvasCtx.moveTo(sortedPoints[i][0] + left, sortedPoints[i][1]);
      canvasCtx.lineTo(sortedPoints[i + 1][0] + left, sortedPoints[i + 1][1]);
      canvasCtx.stroke();
      canvasCtx.fillRect(
      ...
      <React.Fragment>
        ...
        >
          <div style={{ position: "absolute" }}>
            ...
            <span className="col text-center">
              <label htmlFor="duration" style={{ fontSize: "0.8rem" }}>
                Dur:
              </label>
              <input
                type="text"
                ...
                id="duration"
                onChange={e => {
                  changeBlock(this.id, "duration", e.target.value);
                }}
              />
            </span>
            <button
